"""Finance refund agent (LangGraph) with CortexHub.
...
def refund_agent(state: AgentState) -> AgentState:
    ...
    response = refund_llm.invoke([HumanMessage(content=system)] + state["messages"])
...
def main() -> None:
    print("\n" + "=" * 60)
    print("Finance Refund Agent (LangGraph)")
